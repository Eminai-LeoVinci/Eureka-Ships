...
object ShipAssembler {
    // BFS-collect the connected block set (world coordinates) that would become a ship, de-duplicated,
    // or null if it exceeds maxShipBlocks. Deliberately does NOT create the ship, so a caller can inspect
    // (and, for the Eureka Assembler, mutate) the world set and still abort cleanly before anything is
    // built. The BFS can visit a center-neighbour more than once, so counting in [predicate] would
    // over-count; iterate the de-duplicated set returned here for an accurate tally instead.
    fun collectBlockPositions(
        level: ServerLevel,
        center: BlockPos,
        ...
        return HashSet(blocks)
    }

    // Turn a collected block set into a ship. Must run AFTER any pre-assembly world edits (the Eureka
    // Assembler's block swaps), since assembleToShip reads the current world state into the ship. Clears
    // orphaned resting snow first.
    fun finishAssembly(level: ServerLevel, blockPositions: Set<BlockPos>): ServerShip {
        clearRestingSnowLayers(level, blockPositions)
        return assembleToShip(level, blockPositions)
    }
    ...
    // Back-compat one-shot collect-then-assemble (no pre-assembly hook). The helm now drives the two
    // steps directly so it can run the Eureka Assembler in between.
