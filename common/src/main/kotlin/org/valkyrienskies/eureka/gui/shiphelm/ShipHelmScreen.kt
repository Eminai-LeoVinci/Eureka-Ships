...
class ShipHelmScreen(handler: ShipHelmScreenMenu, playerInventory: Inventory, text: Component) :
    ...
    // (streamed to the server, which clamps to what the ship can physically do and syncs the result back).
    // Hidden until wired visible by updateButtons.
    ...
    // Committed on Enter (streamed to the server, which stores it on the player's prefs and resets it to 0 once a
    // ship is assembled). The filter tolerates the trailing % so the displayed value re-parses.
    ...
    private fun commitCruiseBox(axis: Int, box: EditBox) {
        val text = box.value.trim()
        if (text.toDoubleOrNull() != null) sendCruiseValue(axis, text)
        box.isFocused = false
        if (this.focused === box) this.focused = null
    }

    // 1.20.1's container buttonId is only a byte on the wire, so the single wide-id encode 1.21.x uses won't
    // fit. Instead stream the typed value one character at a time over the byte-safe button channel: a clear,
    // then a small id per digit / '.' / '-', then a per-axis commit id. The server (ShipHelmScreenMenu)
    // accumulates the characters and parses on commit. The channel is reliable + ordered, so the burst arrives
    // intact; the whole value goes out at once on Enter, so it's a handful of packets only when the pilot tunes.
    private fun sendCruiseValue(axis: Int, text: String) {
        val gm = minecraft?.gameMode ?: return
        val cid = menu.containerId
        gm.handleInventoryButtonClick(cid, ShipHelmScreenMenu.CRUISE_IN_CLEAR)
        for (c in text) {
            val id = when {
                c in '0'..'9' -> ShipHelmScreenMenu.CRUISE_IN_DIGIT0 + (c - '0')
                c == '.' -> ShipHelmScreenMenu.CRUISE_IN_DOT
                c == '-' -> ShipHelmScreenMenu.CRUISE_IN_MINUS
                else -> continue
            }
            gm.handleInventoryButtonClick(cid, id)
        }
        gm.handleInventoryButtonClick(cid, ShipHelmScreenMenu.CRUISE_IN_COMMIT0 + axis)
    }
    ...
    private fun commitPctBox(which: Int, box: EditBox) {
        val text = box.value.trim().removeSuffix("%")
        if (text.toIntOrNull() != null) sendAssemblerBonus(which, text)
        box.isFocused = false
        if (this.focused === box) this.focused = null
    }

    // Stream the typed % over the byte-safe channel (like sendCruiseValue): a clear, one small id per digit, then
    // the per-which commit id (46 = floater, 47 = balloon). The server (ShipHelmScreenMenu) accumulates + parses.
    private fun sendAssemblerBonus(which: Int, text: String) {
        val gm = minecraft?.gameMode ?: return
        val cid = menu.containerId
        gm.handleInventoryButtonClick(cid, ShipHelmScreenMenu.CRUISE_IN_CLEAR)
        for (c in text) {
            if (c in '0'..'9') gm.handleInventoryButtonClick(cid, ShipHelmScreenMenu.CRUISE_IN_DIGIT0 + (c - '0'))
        }
        gm.handleInventoryButtonClick(
            cid,
            if (which == 0) ShipHelmScreenMenu.ASSEMBLER_IN_COMMIT_FLOATER else ShipHelmScreenMenu.ASSEMBLER_IN_COMMIT_BALLOON
        )
    }
    ...
    private fun updateButtons() {
        ...
        // Keep-active needs an assembled ship AND a VS2 that actually supports it (greyed on official VS2).
        keepActiveCheckbox.active = assembled && menu.keepActiveSupported
    ...
    override fun renderLabels(guiGraphics: GuiGraphics, i: Int, j: Int) {
        ...
        // Bottom-right: engine power (fuel-tank %).
    ...
    // scale the checkbox labels use so the assembler "+" / "Ship's Weight:" text matches the rows around it. Uses
    // the 1.20.1 3D PoseStack (push/scale(s,s,1f)/pop) like drawBoxText below.
    ...
    // Draw dark text centred in a bottom box (panel-relative x), auto-scaled so it fits the box width. The pose
    // is a 3D PoseStack in 1.20.1 (push/scale(s,s,1f)/pop); divide screen coords by the scale to keep position.
