...
class ShipHelmScreenMenu(syncId: Int, playerInv: Inventory, private val blockEntity: ShipHelmBlockEntity?) :
    ...
    companion object {
        ...
        // on the wire in 1.21.x), so no new packet is needed. The client encodes {axis, value} as one large id;
        // the server decodes and clamps. axis: 0 = speed m/s, 1 = turn deg/s, 2 = vertical m/s. Value is
        // fixed-point x100, biased by CRUISE_VALUE_OFFSET so it stays positive within its axis band.
