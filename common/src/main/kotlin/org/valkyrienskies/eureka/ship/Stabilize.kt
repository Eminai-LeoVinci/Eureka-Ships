...
// Vector3d per stabilize call. Everything handed to applyWorldTorque/Force stays freshly
// allocated: vs-core queues those by reference. (1.20.1 port: the 1.21.1 source applied via
// applyInvariantTorque/Force, which are the DEPRECATED names for applyWorldTorque/Force on the
// 1.20.1 vs-core pin — same world-frame application, so translated here to clear the warnings.)
...
fun stabilize(
    ...
    forces.applyWorldTorque(stabilizationTorque)
    ...
    if (linear) {
        ...
        forces.applyWorldForce(idealVelocity)
