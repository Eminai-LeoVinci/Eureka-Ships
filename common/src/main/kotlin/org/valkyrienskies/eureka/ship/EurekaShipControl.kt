...
// 1.20.1 port note: the 1.21.1 source of this class reads angular velocity via `physShip.omega` and
// applies forces via `applyInvariantForce`/`applyInvariantTorque`. On the 1.20.1 vs-core pin the PhysShip
// property is `angularVelocity` and the world-frame force appliers are `applyWorldForce`/`applyWorldTorque`
// (both proven by the stock 1.20.1 EurekaShipControl this replaces). The overhaul physics are otherwise
// verbatim. 1.20.1 core also gates several ship/phys APIs behind @RequiresOptIn markers (GameTickOnly /
// PhysTickOnly / VsBeta), opted in at the class level below.
@JsonAutoDetect(
...
class EurekaShipControl : ShipPhysicsListener, ServerTickListener {
    ...
    // them). Only intermediates live here — every vector handed to applyWorldForce/Torque
    ...
    private fun applyWaterAltitudeHold(
        ...
        physShip.applyWorldForce(Vector3d(0.0, appliedY, 0.0))
    }
    ...
    private fun applyPlayerControl(control: ControlData, physShip: PhysShip, thrustMultiplier: Double) {
        ...
        physShip.applyWorldTorque(moiTensor.transform(Vector3d(0.0, idealAlphaY, 0.0)))
        // endregion

        physShip.applyWorldTorque(getPlayerControlledBanking(control, physShip, moiTensor, -idealAlphaY))

        physShip.applyWorldForce(getPlayerForwardVel(control, physShip).mul(thrustMultiplier))
    }
