...
/**
 * Reads `config/vs_eureka.json` and copies its values into [EurekaConfig.ADVANCED] ("server"),
 * [EurekaConfig.VANILLA] ("serverVanilla") and [EurekaConfig.CLIENT] ("client") at mod init. VS 2.5
 * vs-core removed `registerConfig`, so this is Eureka's replacement until VS2's ModConfigSpec
 ...
 *
 * Behavior:
 * - File missing: write defaults from the current ADVANCED / VANILLA / CLIENT singletons.
 ...
 * - Legacy files (no "serverVanilla" key): VANILLA keeps its baked 833d445 defaults and the
 *   re-write below emits the key populated, so old configs auto-migrate.
 */
    ...
    fun loadOrCreate() {
        try {
            ...
            tree.get("server")?.takeIf { !it.isMissingNode && !it.isNull }?.let {
                mapper.readerForUpdating(EurekaConfig.ADVANCED).readValue<Any>(it)
            }
